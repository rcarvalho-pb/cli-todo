...
SELECT id, title, is_completed, created_at, completed_at FROM tb_tasks WHERE title LIKE CONCAT('%', ?, '%') ORDER BY created_at DESC
`
...
SELECT id, title, is_completed, created_at, completed_at FROM tb_tasks WHERE is_completed = 1 ORDER BY completed_at DESC
`
...
SELECT id, title, is_completed, created_at, completed_at FROM tb_tasks ORDER BY is_completed, case when completed_at IS NOT NULL THEN completed_at else is_completed end DESC
`
